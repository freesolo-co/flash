...
- Thinking deployments now verify the mutable run alias after activation with the same bounded
  request shape as the immutable revision smoke. The check retries transient cold starts, requires
  exact revision and checkpoint provenance, and accepts only direct reasoning metadata. A
  post-activation verification failure remains a known active but failed alias target for safe
  redeploy compare-and-swap instead of being reported ready or left activation-unknown.
