...
- OpenAI chat serving now supports strict function tool calling for exact Qwen3.5 packaged and
  hosted engines, including validated tool history, buffered and raw SSE `tool_calls`, and managed
  forwarding. Flash parses schema-valid calls but does not execute them; malformed automatic calls
  remain exact assistant text.
