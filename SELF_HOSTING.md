...
| `FLASH_API_URL`        | On-policy distillation (`opd`) submissions fail before GPU allocation. This is **this plane's own public origin**, which is both where your CLI dials in and where a GPU worker dials back to reach the teacher broker. Set it to an origin a rented worker can resolve: if you reach your plane over a tunnel, a VPN, or `localhost`, that address is not one, and only the worker's failure to present a capability - after the GPU is allocated - will tell you. `sft` and `grpo` do not use it server-side. |
