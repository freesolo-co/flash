...
| `FLASH_CONTROL_PANEL_URL` | Falls back to `FLASH_API_URL`; OPD submissions fail before GPU allocation only when neither is set. This is **this plane's own public origin** - the address a GPU worker dials back to reach the teacher broker, which is the same origin your CLI talks to whenever the plane is publicly addressed at that name. Set it explicitly when it is not: if you reach your plane over a tunnel, a VPN, or `localhost`, `FLASH_API_URL` is your client's address, not a worker's, and only the failure to present a capability after the GPU is already allocated will tell you. |
