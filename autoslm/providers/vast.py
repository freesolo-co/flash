...
The Vast substrate has no serverless queue: we rent a single-GPU instance from a
VERIFIED DATACENTER offer, ship a self-contained bootstrap (``vast_bootstrap.py``)
through the onstart script, and detect completion purely via the worker's HF
artifacts (DONE/metrics.json/heartbeat.json) + the instance's status — no inbound
network to the box is ever needed.
...
from autoslm.flash.durable import PollResult, make_hf_heartbeat_reader
from autoslm.flash.gpus import GPU_INFO, min_cuda_modern, vast_gpu_for_offer
...
def build_onstart(payload: dict, install_deps: bool = True) -> str:
    ...
    """
    from autoslm.flash.train import resolve_worker_deps

    payload_b64 = base64.encodebytes(json.dumps(payload).encode()).decode()
    bootstrap_src = (Path(__file__).parent / "vast_bootstrap.py").read_text()
    if install_deps:
    ...
    return f"""#!/bin/bash
# AutoSLM vast worker (generated by autoslm.providers.vast.build_onstart)
set -x
...
def deploy_and_submit(
    spec, seed: int, offers: list[VastOffer], attempt: int = 0, log=None
) -> VastJobHandle:
    ...
    while candidates:
        ...
        try:
            instance_id = vast_api.create_instance(
                ...
                disk_gb=max(float(spec.gpu.disk_gb), 60.0),
                env={},
                onstart=onstart,
                label=label,
                runtype="args",
            ...
            if not candidates and not refreshed:
                ...
                candidates = [
                    ...
                    for o in usable_offers(
                        min(o.vram_gb for o in offers), spec.gpu.disk_gb, exclude_machine_ids=taken
                    )
                    if o.gpu in allowed
                ][:5]
            continue
...
def submit_train_durable_vast(
    ...
    """Vast equivalent of ``durable.submit_train_durable``: rent, persist handle, poll.
    ...
    if offers is None:
        ...
        offers = [
            o for o in usable_offers(info.vram_gb, spec.gpu.disk_gb) if o.gpu == spec.gpu.type
        ]
    handle = deploy_and_submit(spec, seed, offers, attempt=attempt, log=log)
