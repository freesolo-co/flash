...
# Instance-provider WARM (Lambda). RunPod warms via the serverless preload above; the instance
# providers have no serverless API, so a warm is a real (cheap, short) GPU launch in download-only
# mode: the bootstrap pulls the catalog into the mounted cache and exits (no worker). The box
...
def warm_instances(models: list | None = None, gpu: str | None = None,
    ...
    """WARM the Lambda cache: one download-only launch per region that currently has
    ...
    # Per-provider "can this region host the cache?" predicate. Skipping a cache-incapable region
    # BEFORE launching avoids burning a paid GPU whose preload just reports "weight cache not supported
    # in region" — which main() then counts as a failed warm. Lambda exposes no such filter (every
    # region hosts filesystems), so it stays unfiltered.
    region_ok: dict = {}
...
def provision_all() -> list[str]:
    """Eagerly create the cache storage on every instance provider, in every region
    ...
    volume-create in the SDK. Returns ``provider:<region>`` per storage created/confirmed."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="WARM the Lambda cache: one download-only GPU launch per region with "
    ...
    if args.provision:
        # Eagerly create the instance-provider cache storage in every region (GPU-free). RunPod's
    ...
    if args.warm_instances:
        if args.dry_run:
            print("would warm Lambda cache (one download-only launch per region with capacity)")
            return 0
        ...
        if not results:
            # NOT the same as "warmed everything": zero launch targets means no Lambda region had
            # capacity to warm right now (or every candidate region is cache-incapable — each such
            # skip is logged above). This is a best-effort no-op, not a failure: those regions'
            # weights simply download cold on first run. Make it explicit so "0/0" isn't read as
            # success. (See the per-region "skipping cache-incapable region" / "no capacity" logs.)
    ...
    if args.teardown:
        ...
        # Each provider is guarded INDEPENDENTLY so one provider's failure (e.g. RunPod auth
        # absent/broken on an instance-only control plane, or a RunPod outage) never aborts the
        # others' best-effort cleanup — otherwise their billed caches would leak behind a single
        # RunPod error. A provider with no configured key is already a no-op.
        deleted: list[str] = []
        try:
        ...
        # `--datacenters` is a RunPod-DC subset and has no meaning for the instance provider (Lambda
        # regions are a different namespace), so a SCOPED teardown stays RunPod-only rather than
        # unexpectedly deleting every Lambda `flash-weights` cache too. Only a FULL teardown (no
        # --datacenters) reclaims the instance-provider caches.
        if not scoped:
            try:
