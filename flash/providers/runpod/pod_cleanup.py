...
from flash.providers._lifecycle.instance import label_matches_run
from flash.providers._lifecycle.poll import preload_box_reap_due
...
def _pod_payload_secret_name(pod: runpod_api.RunpodPod) -> str:
    ...
    for fingerprint, pods in by_fingerprint.items():
        for pod in pods:
            if not label_matches_run(pod.name, prefix):
                continue
            _account, secrets = runpod_api.list_secrets_for_fingerprint(
                fingerprint,
                name=_pod_payload_secret_name(pod),
                deadline_at=deadline_at,
            )
            runpod_api.delete_pod_for_fingerprint(pod.id, fingerprint, deadline_at=deadline_at)
