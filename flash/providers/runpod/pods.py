...
from flash.providers._lifecycle.instance import _spill_large_spec_to_hf
...
from flash.providers.runpod.pod_identity import (
    EXACT,
    POD_CREATE_PENDING,
    PRE_POD_CREATE,
...
def _new_secret_intent(
    ...
    *,
    ...
    for _ in range(_CREATE_RECONCILE_POLLS):
        ...
        account_id, existing = runpod_api.list_secrets_for_fingerprint(
...
def _create_payload_secret(
    ...
    *,
    ...
    try:
        secret = runpod_api.create_secret_for_fingerprint(
        ...
        for poll in range(_CREATE_RECONCILE_POLLS):
            account_id, observed = runpod_api.list_secrets_for_fingerprint(
                intent.key_fingerprint,
    ...
    if secret.name != intent.payload_secret_name:
        raise runpod_api.RunpodMutationAmbiguous("runpod payload secret identity is unknown")
...
def _exact_handle(pending: RunpodPodHandle, pod: runpod_api.RunpodPod) -> RunpodPodHandle:
    return replace(
        pending,
        instance_id=pod.id,
        phase=EXACT,
...
def _delete_duplicate_pods(
    fingerprint: str,
    pods: list[runpod_api.RunpodPod],
    keep_id: str,
    *,
    deadline_at: float,
) -> None:
    for duplicate in pods:
        if duplicate.id == keep_id:
            continue
        runpod_api.delete_pod_for_fingerprint(duplicate.id, fingerprint, deadline_at=deadline_at)
...
def _reconcile_ambiguous_create(
    ...
    *,
    ...
    for poll in range(_CREATE_RECONCILE_POLLS):
        observed = runpod_api.list_pods_for_key(
        ...
        if observed:
            for pod in observed:
                with contextlib.suppress(Exception):
                    runpod_api.delete_pod_for_fingerprint(
...
def create_or_adopt_pod(
    ...
    *,
    ...
    try:
        pod = runpod_api.create_pod_for_fingerprint(
            pending.key_fingerprint,
            payload,
            deadline_at=deadline_at,
        )
        ...
        fetch_instance=lambda: (
            ...
            if (
                pod := runpod_api.get_pod_for_fingerprint(
                    handle.pod_id,
                    handle.key_fingerprint,
                    **deadline_kwargs(runpod_api.get_pod_for_fingerprint, absolute_deadline),
                )
            )
            is None
...
def _delete_payload_secret(handle: RunpodPodHandle, *, deadline_at: float) -> None:
    if handle.payload_secret_id is None:
        ...
        secret = runpod_api.RunpodSecret(handle.payload_secret_id, handle.payload_secret_name)
    runpod_api.delete_secret_for_fingerprint(
        handle.key_fingerprint,
        secret.id,
        secret.name,
        deadline_at=deadline_at,
...
def terminate_handle(handle: RunpodPodHandle, *, deadline_at: float) -> None:
    if handle.phase == EXACT:
        ...
        key = runpod_api._key_for_fingerprint(handle.key_fingerprint)
        matching = runpod_api.list_pods_for_key(
            key, keep_name=handle.label, deadline_at=deadline_at
        )
        for pod in matching:
            runpod_api.delete_pod_for_fingerprint(
        ...
        if runpod_api.list_pods_for_key(key, keep_name=handle.label, deadline_at=deadline_at):
...
def launch_payload_pod(
    ...
    *,
    ...
    try:
        ...
        except (runpod_api.RunpodMutationAmbiguous, UnreconciledCreateError):
            uncertain = True
            raise
        if on_handle is not None:
...
from flash.providers.runpod.pod_cleanup import (  # noqa: E402,F401
    destroy_run_pods,
    run_pods_remaining,
    sweep_orphan_pods,
)
