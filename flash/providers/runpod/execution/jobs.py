...
def capacity_wait_kwargs(on_last_gpu: bool = False, gpu_count: int = 1) -> dict:
    """return queue and throttling observation windows for one rented shape."""
    grace = (LAST_GPU_CAPACITY_GRACE_S if on_last_gpu else 300.0) * capacity_grace_multiplier(
        gpu_count
    )
    return {"queue_grace_s": grace, "throttled_grace_s": grace}
...
class JobHandle:
    ...
    def from_dict(cls, d: dict) -> JobHandle:
        ...
        if isinstance(fence, bool) or not isinstance(fence, int) or fence < 1:
            raise ValueError("persisted RunPod fence identity is invalid")
        ...
        return cls(endpoint_id, endpoint_name, fingerprint, job_id, attempt, fence, started_ts)
