...
def realized_cost_for_remote(
    ...
    Two distinct time bounds, because the two cost sources are different:
      ...
      * ``run_end`` is the run's ACTUAL terminal time (~teardown). The instance provider (Lambda)
        has no billing endpoint: an instance bills at a flat $/hr from launch to teardown, so its
        realized COGS is wall x rate over ``started_ts -> run_end`` — it must NOT use the
        settle-padded ``end`` or it would over-bill by the padding (up to an hour). Defaults to
        ``end`` for back-compat when the caller doesn't distinguish.
    """
    if not remote:
        return None
