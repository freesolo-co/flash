...
_REMOVED_OPTIMIZATION_ENV = frozenset(
    {
        # chalk's kernels were applied to an in-process trainer.model, which the verl backends no
        # longer have (both train out-of-process), so the install-source override selects kernels
        # nothing can engage. Warn rather than let a recipe key silently do nothing.
        "FLASH_CHALK_SPEC",
        "PYTORCH_ALLOC_CONF",
