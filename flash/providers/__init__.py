...
owns pricing, provisioning, polling, cancellation, and teardown.
  ...
  lambda  Lambda Cloud GPU instances (instance-based complement; available iff LAMBDA_API_KEY set)
...
without ``LAMBDA_API_KEY`` silently behaves exactly as the RunPod-only setup).
"""
...
# Active provider order is also the tie-break preference (RunPod wins price ties).
