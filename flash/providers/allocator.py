...
RunPod's cheaper static rates almost always win on price, so the instance provider (Lambda) joins
the ranked list as a capacity COMPLEMENT: when RunPod's cheapest fitting class is out of capacity
(THROTTLED / queue backstop), the runner's gpu-walk steps down the ranked list and reaches an
in-capacity Lambda class. Lambda is capacity-filtered up front (``_lambda_candidates`` only offers
a class a region can supply right now), so the walk never lands on a class that would just fail to
launch.
...
def allocate(
    ...
    *,
    ...
    (Lambda via LAMBDA_API_KEY — opt-in) contributes its fitting classes that currently have live
    capacity. RunPod's cheaper static rates usually win, with Lambda joining as a capacity
    complement lower in the ranked list.
