...
# the threshold regression builds the real payload shape, including deadlines, cache fields, code
# identity, environment, and attempt identity, then pins the worst inline case below 62,000 bytes.
...
def _render_user_data(payload: dict, *, image: str) -> str:
    ...
    # Shipped Python modules are stripped of docstrings on the way in. They are for the reader of the
    ...
    # the host-log uploader starts before image pull so an instance that reached cloud-init leaves
    # liveness evidence while the multi-gb image is still downloading. the poller distinguishes that
    # from a silently dead instance and can fail over before the 3000s setup grace. docker inspect is
    # intentionally the stop condition: before the container exists, "not running" means not started,
    # not finished. the 120s cadence and 15-iteration bound protect the shared-repo commit budget.
    #
    # after launch, a stopped container is success only at exit 0. the worker owns its attempt marker;
    # the host must not overwrite an ok marker or relabel a user/config failure as retriable. failmark
    # checks for the worker marker and writes only when the container never produced one.
    return f"""#!/bin/bash
...
# a stopped container succeeds only at exit 0; failmark preserves any worker-owned marker.
