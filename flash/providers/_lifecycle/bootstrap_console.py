...
def _run_console_upload_loop(console: str, interval_s: float, stop_upload, *, upload) -> None:
    ...
    promotion is read after ``due`` so it only moves the next deadline -- a 600-second deadline that
    already expired with nothing committed still spends its snapshot, which is the startup evidence
    it exists for. failed uploads change no upload state, so the next poll retries.
    """
