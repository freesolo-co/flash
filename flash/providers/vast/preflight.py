...
problem list the control plane aggregates into one startup error. Vast is opt-in (the
allocator only reaches for it when ``VAST_API_KEY`` is set), so the only Vast-specific
requirement is ``VAST_API_KEY``; HF_TOKEN is a shared run requirement checked once
centrally by the cross-provider preflight (``flash/providers/preflight.py``), which calls
each provider-specific check with ``require_hf=False`` so HF is never double-reported (the
HF dataset repo is per-run via ``[train] hf_repo``).
"""
