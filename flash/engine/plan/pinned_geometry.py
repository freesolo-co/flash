"""Certification of a PINNED model revision's geometry against its catalog row.
...
A pinned revision names immutable weights, so the question every caller here asks is the same one:
does the commit the worker will actually load agree with the catalog row describing the model? The
answers differ only in policy -- sizing must fail closed, the GPU-width cap degrades to a
conservative ceiling -- so the rule is defined once in `_certify_revision_geometry`.
"""
...
def _certify_revision_geometry(model_id: str, revision: str, info) -> tuple:
...
def _validated_revision_geometry(model_id: str, revision: str, info):
    params_b, vocab, _hidden, _layers, _heads = _certify_revision_geometry(model_id, revision, info)
...
def certified_revision_attention_heads(model_id: str, revision: str) -> int:
    ...
    Returns 0 -- "not certified" -- for everything `_certify_revision_geometry` will not vouch for:
    ...
    if info is None or not revision:
        return 0
    try:
        _params_b, _vocab, _hidden, _layers, heads = _certify_revision_geometry(
            model_id, revision, info
        )
    except Exception:
        return 0
