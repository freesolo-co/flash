...
def _prepare_sft_workload_inputs(
    spec,
    env,
    *,
    model_revision: str,
...
def _resolve_sft_inputs(
    ...
    *,
    ...
    workload = _prepare_sft_workload_inputs(
        spec,
        env,
    ...
    per_device_batch, _ = sft_grad_accum(
        effective_batch,
        seq_len=workload.realized_max_length,
        vocab=vocab_size,
        fused=fused_ce,
    )
    train_batch_size = workload.profile.examples_per_update
    micro_batch = max(1, min(per_device_batch, train_batch_size))
    steps_per_epoch = max(1, math.ceil(len(workload.rows) / train_batch_size))
    update_horizon = workload.profile.authoritative_steps
    ...
    verl_setup = _resolve_sft_verl_setup(
        spec,
        model_id=model_id,
        model_revision=model_revision,
        workdir=workdir,
    ...
    return _SftResolvedInputs(
        ...
        python_bin=verl_setup.python_bin,
        caps=verl_setup.caps,
        model_path=verl_setup.model_path,
        loggers=verl_setup.loggers,
        project_name=verl_setup.project_name,
        experiment_name=verl_setup.experiment_name,
        shim_dir=verl_setup.shim_dir,
        custom_dataset_path=verl_setup.custom_dataset_path,
        gdn_hybrid=verl_setup.gdn_hybrid,
        gdn_reset_arch=verl_setup.gdn_reset_arch,
    )
...
def _build_sft_config(inputs: _SftResolvedInputs, use_remove_padding: bool) -> dict[str, Any]:
    return {
        ...
        "max_length": inputs.workload.max_length,
        "micro_batch": inputs.micro_batch,
        "max_token_len_per_gpu": inputs.workload.realized_max_length * inputs.micro_batch,
        "custom_dataset_path": inputs.custom_dataset_path,
        "model_path": inputs.model_path,
        "lora_rank": inputs.lora_rank,
        "lora_alpha": inputs.lora_alpha,
        "target_modules": inputs.target_modules,
        "target_parameters": _w.lora_target_parameters(inputs.model_id),
        "lora_adapter_path": inputs.warmstart_adapter,
        "ulysses_sp_size": inputs.gpu_count,
        ...
        "n_gpus_per_node": inputs.gpu_count,
        "seed": _w.backend_seed(_w.SEED),
        "project_name": inputs.project_name,
        "experiment_name": inputs.experiment_name,
        "loop_epochs": inputs.loop_epochs,
        "loggers": inputs.loggers,
        ...
        "gradient_checkpointing": (
            inputs.gradient_checkpointing and not inputs.reentrant_gradient_checkpointing
        ),
        "total_training_steps": inputs.update_horizon if inputs.max_steps > 0 else None,
...
def _prepare_sft_child(inputs: _SftResolvedInputs, config: dict[str, Any]) -> _SftChildSetup:
    ...
    shim_source = _render_sft_sitecustomize(
        ...
        total_steps=inputs.update_horizon,
        reentrant_gradient_checkpointing=inputs.reentrant_gradient_checkpointing,
    )
    ...
    if inputs.gdn_reset_arch is not None:
        shim_source += render_gdn_varlen_shim(inputs.gdn_reset_arch)
    if "wandb" in inputs.loggers:
        shim_source += render_wandb_link_shim()
    with open(os.path.join(inputs.shim_dir, "sitecustomize.py"), "w", encoding="utf-8") as file:
        file.write(shim_source)
    with open(inputs.custom_dataset_path, "w", encoding="utf-8") as file:
        file.write(_render_sft_dataset_module())

    resume_step = _restore_verl_resume(inputs.local_dir)
    watcher = _VerlCheckpointWatcher(
        local_dir=inputs.local_dir,
        export_root=inputs.export_root,
        python_bin=inputs.python_bin,
        model_id=inputs.model_id,
        model_revision=inputs.model_revision,
        required_steps=inputs.save_at_steps,
    )
    watcher.processed_steps.update(_durable_required_save_steps(inputs.save_at_steps, resume_step))
    if resume_step >= inputs.update_horizon:
        missing = sorted(watcher.required_steps - watcher.processed_steps)
        if missing:
            raise RuntimeError(f"required saves were not durably published: {missing}")

    child_env = _build_verl_child_env(
        shim_dir=inputs.shim_dir,
        wandb_enabled="wandb" in inputs.loggers,
    )
    command = [
        inputs.python_bin,
        "-m",
        ...
        "--standalone",
        "--nnodes=1",
        f"--nproc-per-node={inputs.gpu_count}",
        "-m",
        "verl.trainer.sft_trainer",
        *overrides,
    ]
    return _SftChildSetup(
...
def _new_sft_runtime_state(
    inputs: _SftResolvedInputs,
    child: _SftChildSetup,
) -> _SftRuntimeState:
    progress = {"step": child.resume_step, "loss": None, "grad_norm": None, "lr": None}
    ...
    train_tokens = (
        sft_tokens_for_updates(
            inputs.workload.rows,
            examples_per_update=inputs.train_batch_size,
            updates=child.resume_step,
            field="input_ids",
        )
        if child.resume_step > 0
        else 0
    )
    return _SftRuntimeState(
        progress=progress,
        zero_grad_steps=zero_grad_steps,
        observed_grad_norms=observed_grad_norms,
        loss_curve=loss_curve,
        train_tokens=train_tokens,
        loraplus_applied=child.resume_step >= inputs.update_horizon,
        wandb_link={},
    )
...
def _run_sft_child(
    ...
    *,
    ...
    return_code = 0
    if child.resume_step < inputs.update_horizon:
        child.watcher.start()
    ...
    if _SFT_LORAPLUS_RATIO > 1 and not runtime.loraplus_applied:
        raise RuntimeError("required lora+ shim did not emit its success marker")

    actor_dir, final_step = latest_global_step_dir(inputs.local_dir)
    if sft_under_ran(final_step, inputs.update_horizon, inputs.max_steps):
        raise RuntimeError(
            f"sft completed {final_step}/{inputs.update_horizon} requested optimizer updates"
        )
    ...
    if (
        not child.resume_step
        and runtime.observed_grad_norms
        and not any(runtime.observed_grad_norms)
    ):
        raise RuntimeError(
            "verl reported train/grad_norm=0.0 on every one of "
            f"{len(runtime.observed_grad_norms)} observed optimizer updates: no gradient is reaching "
            "the lora parameters, so this run would train nothing. see GRAD-001"
        )
    runtime.train_tokens = sft_tokens_for_updates(
        inputs.workload.rows,
        examples_per_update=inputs.train_batch_size,
        updates=final_step,
        field="input_ids",
...
def _write_sft_train_meta(
    inputs: _SftResolvedInputs,
    child: _SftChildSetup,
    runtime: _SftRuntimeState,
    result: _SftTrainResult,
) -> None:
    _w.write_train_meta(
        phase="sft",
        ...
        setup_seconds=inputs.setup_seconds,
        train_tokens=runtime.train_tokens,
        generated_tokens=0,
        step=result.final_step,
        notes={
            "epochs": inputs.epochs,
            "resumed": bool(child.resume_step),
            "warm_started": bool(inputs.warmstart_adapter),
            "download_seconds": inputs.download_seconds,
            "hf_transfer": os.environ.get("HF_HUB_ENABLE_HF_TRANSFER", ""),
            "thinking": _w.THINKING,
            "multimodal": inputs.workload.multimodal,
            "gradient_checkpointing": inputs.gradient_checkpointing,
            "gradient_checkpointing_reentrant": inputs.reentrant_gradient_checkpointing,
            "configured_max_length": inputs.workload.max_length,
            "realized_max_length": inputs.workload.realized_max_length,
            "runtime_max_length": inputs.workload.realized_max_length,
            "per_device_train_batch_size": inputs.micro_batch,
            "gradient_accumulation_steps": math.ceil(inputs.train_batch_size / inputs.micro_batch),
            ...
            "packing": inputs.workload.profile.packing_mode,
            ...
            "gdn_boundary_resets": (
                (inputs.gdn_reset_arch is not None) if inputs.gdn_hybrid else None
            ),
            "loss_curve": runtime.loss_curve[:400],
            "peak_gpu_gb": result.device_peak_gpu_gb,
            "device_peak_gpu_gb": result.device_peak_gpu_gb,
            "loraplus_optim": _VERL_OPTIMIZER_NAME,
            "loraplus_applied": runtime.loraplus_applied,
            "verl_backend": "fsdp2",
            "ulysses_sequence_parallel_size": inputs.gpu_count,
            "wandb_project": inputs.project_name if "wandb" in inputs.loggers else None,
            "wandb_run_name": inputs.experiment_name if "wandb" in inputs.loggers else None,
            ...
            "wandb_url": runtime.wandb_link.get("wandb_url"),
            "wandb_id": runtime.wandb_link.get("wandb_id"),
        },
    )
...
def run_sft_train(spec=None) -> None:
    ...
    config = _build_sft_config(inputs, use_remove_padding)
    child = _prepare_sft_child(inputs, config)
    runtime = _new_sft_runtime_state(inputs, child)

    def on_line(line: str) -> None:
        child.watcher.raise_if_failed()
        if _LORAPLUS_READY_MARKER in line:
            runtime.loraplus_applied = True
        link = parse_wandb_link(line)
        if link is not None:
            runtime.wandb_link.update(link)
