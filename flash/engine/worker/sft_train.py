...
_CHILD_ENV_EXACT = frozenset(
    {
        ...
        "FLASH_VERL_PYTHON",
        # the baked per-sm kernel cache (Dockerfile.worker) lives at /opt/flash/kernelcache and is
        # addressed ONLY through these five vars. the child is the interpreter that actually trains,
        # so a cache the child never sees is no cache at all: without them it falls back to
        # $HOME/.cache and re-JITs the FA2/GDN triton kernels the bake already paid ~124s to build.
        # listed EXACTly rather than by prefix because none of them share one -- note
        # TORCHINDUCTOR_CACHE_DIR does NOT match the "TORCH_" prefix below (its sixth character is
        # "I"), which is why it was dropped alongside the others despite looking like it passes.
        # safe to share: /opt/verl-venv resolves the SAME torch as the parent (both 2.10.0+cu128 on
        # the published cu128-sm90 image), and both are image-local, so this shares nothing across
        # tenants -- see providers/_lifecycle/worker.py, which deliberately keeps JIT caches OFF the
        # multi-tenant volume.
        "TRITON_CACHE_DIR",
