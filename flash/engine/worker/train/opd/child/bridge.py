...
def _write_child_failure_fallback(
    ...
    # the SAME file the teacher-path exits write, because the parent reads exactly one fallback per
    # run and both records answer the same question: why did an agent-loop actor die without its
    # exit code reaching the driver. a second path here would be written by the child and read by
    # nobody, which is the failure this fallback exists to prevent.
    _write_failure_fallback(
