...
# the source string for a reply arriving DURING an episode, exported so the bridges name it rather
# than retyping it: the image diagnosis below keys on it, and a caller who typed "env reply" would
# silently get the generic message instead. every other source is an initial prompt, where images
# ARE supported and "put the image in the initial prompt" would be circular advice.
ENVIRONMENT_REPLY_SOURCE = "environment reply"
...
def validate_transcript_messages(messages: list[dict], *, source: str) -> list[dict]:
    ...
    for position, message in enumerate(messages):
        ...
        if not isinstance(content, str):
            if _has_image_block(content) and source == ENVIRONMENT_REPLY_SOURCE:
