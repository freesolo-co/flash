...
def recombine_lora_adapters(
    ...
    # The VL merge warm-start (#296) trains the prior adapter against the FULL multimodal model, so
    # the warm-start adapter's keys carry the infix (``base_model.model.model.language_model.layers
    # ...``), while the fresh LoRA is saved by the text-only ``AutoModelForCausalLM`` trainer with no
    # infix (``base_model.model.model.layers...`` — see _remap_vl_sync_weights / warm-start remap note).
