...
def _apply_opd_optimizer_update(
    *,
    ...
    Reverse-KL gradients are corrected to the valid-sequence mean first. When a hybrid run supplies
    forward_backward, it runs AFTER that reverse correction and BEFORE the single combined clip, so its
    independently normalized projected-soft gradient is added on top without being rescaled by the
    reverse accum_target/nseq correction. The combined gradient is clipped once and the optimizer
    steps once.
    """
...
def run_opd(forward_teacher_accounting):
    ...
    # hybrid-only forward telemetry, reset fresh each run: it is not threaded through full-state
    # resume (out of this feature's scope). the reverse loss_curve above stays resume-consistent so
    # the honest opt_steps invariant (loss_curve length == opt_steps) holds; on the rare hybrid+resume
    # path these auxiliary curves restart empty.
    ...
    with (
        ...
        while opt_steps < steps and step < max_iters:
            ...
            # failed / empty completion), the mutation boundary rescales to a true 1/nseq mean so a
            # partial step isn't a silently smaller update. a hybrid run injects the projected-soft
            # forward backward between that reverse rescale and the single combined clip, so the forward
            # gradient is added on top without being rescaled by the reverse accum_target/nseq factor.
            ...
            if hybrid.enabled:
                ...
                def _run_forward_backward(_targets=forward_targets):
                    nonlocal forward_loss, supervised_position_count
                ...
                forward_backward = _run_forward_backward
            optimizer_started = time.perf_counter()
            forward_seconds_before = opd_phase_seconds["forward_objective_forward_backward"]
            ...
            # the projected-soft forward backward runs inside the mutation boundary; keep its measured
            # time in forward_objective_forward_backward only so optimizer_step stays the clip+step cost.
            opd_phase_seconds["optimizer_step"] += (
                time.perf_counter()
                - optimizer_started
