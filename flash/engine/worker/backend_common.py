...
# `_ulysses_flash_attention_forward` -- it patches `_flash_attention_forward` and nothing else -- so at
# a width > 1 the decoder hands each rank's SLICE straight to `self.linear_attn` with no gather, and
# every rank but rank 0 starts its recurrence mid-sequence from zero state. Measured 21% relative
# divergence on a 4-layer text model at width 2, with rank 0 bit-identical. Packed boundary resets
# cannot save it: ulysses slices at fixed `seqlen // width` offsets, blind to example boundaries.
#
