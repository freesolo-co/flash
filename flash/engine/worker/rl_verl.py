...
class MultiTurnBridge:
    ...
    def score(self, payload: dict) -> dict:
        ...
        with self._lock:
            session = entry["session"] = self._session(payload)
            state = session["state"]
            # snapshotted here, under the same lock that guards the session and BEFORE parking for
            # the batch below: `step` mutates this list in place, and a concurrent episode's turn
            # would otherwise be read mid-append. taking it before the wait also keeps the sample
            # describing this episode as it finished rather than as it looked after the drain.
