...
def _refresh_cancellation_result(run_id: str, effective_spec) -> None:
    ...
    if status.attempt is None:
        ...
        # here would abort the cancel between teardown and billing. a malformed record is a
        # different fact and still raises: `actual_steps_run` matches billing against the raw
        # dict without validating it, so skipping the refresh would price a cancel off an
