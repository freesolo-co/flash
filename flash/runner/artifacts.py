...
def _pin_env_sha_with_reason(spec: JobSpec) -> tuple[JobSpec, str]:
    ...
    pinned, error = _resolve_environment_sha_once(spec, timeout=10.0)
    if error is None:
        return pinned, ""
...
def preflight_validate_environment_ref(spec: JobSpec) -> JobSpec:
    """reject an authenticated 404/422 before billing or allocation.

    transient and unclassified failures keep the existing worker deferral. tokenless planes skip
    the gate because github also returns 404 for private repositories an anonymous caller cannot
    ...
    pinned, error = _resolve_environment_sha_once(spec, timeout=4.0)
