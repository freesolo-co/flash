...
- **SFT records must be text-only.** Flash quotes every SFT run from a workload profile that
  ...
  rejected too; `flash env test --algorithm sft` refuses both locally, before you submit. The
  image-capable models are still image-capable: train them on images with `grpo` or `opd`, whose
  rollouts tokenize on the GPU worker.
