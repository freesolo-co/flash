...
- **Multi-turn targets train on the assistant turns only.** When `sft_completion` returns a
  ...
  In such multi-turn targets, ChatML control strings `<|im_start|>` and `<|im_end|>` are reserved
  in message content because they are indistinguishable from structural turn delimiters after
  tokenization.
