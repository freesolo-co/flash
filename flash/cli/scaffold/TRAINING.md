...
| Knob                           | Convention                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `group_size`                   | Completions sampled per prompt. GRPO accepts exactly `2`, `4`, or `8` and defaults to `8`; Flash never rounds or downsizes an authored value. More completions can improve the within-group comparison but multiply rollout cost.                                                                                                                                                                                 |
...
| `prompts_per_step`             | The optimizer prompt batch. It defaults to `64`; Flash preserves an authored positive integer exactly and never rounds or downsizes it. Too small and the reward trend is pure noise.                                                                                                                                                                                                                             |
...
`prompts_per_step * group_size <= 512`; the exact ceiling passes and the first value above it is
rejected before GPU lookup or allocation. Rollout generation and reward work, and therefore cost,
multiply by this product. Flash does not rescue an oversized request by rounding the group, reducing
the prompt batch, or relying on a small retained dataset.
