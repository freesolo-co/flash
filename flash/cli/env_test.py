...
def _run_rollout(env, example: dict, turn_content) -> tuple[dict, list[str], list[dict], bool]:
    ...
    while True:
        ...
        if turns >= hard_cap or env.rollout_done(state, max_turns=hard_cap):
            break
        # the locally-built transcript, not state["messages"]: see the note above. an env that
        # keeps its conversation elsewhere still gets the same messages the run would show it.
    ...
    #
    # it belongs here rather than at the scoring pass because scoring now runs as one batch over
