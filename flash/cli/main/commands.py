...
def _cmd_train_cost(args) -> int:
    ...
    ``estimate_cost`` sizes VRAM offline (``skip_net=True``), so the quote is deterministic for
    a listed model. An unlisted ``model_policy = "allow"`` model still reads its param count from
    HF during the spec parse (the same probe the submit path uses), falling back to the heuristic
    when unreachable. For an SFT run with no ``[train].max_examples`` the step count loads the env
    to count its real train split (see ``cost.spec.count_env_examples``).
    """
