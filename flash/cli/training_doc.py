...
- **SFT is a great warm start for GRPO.** SFT first to teach the format and a competent
  ...
  for deployment, so `SFT rank + GRPO rank` must stay within the serving rank cap
  (32 today). A rank-16 SFT plus rank-16 GRPO is deployable; rank 32 plus rank 32 is not.
...
lora_rank = 16     # for VL warm-starts, SFT rank + GRPO rank must be <= 32
lora_alpha = 32
```
