...
_LITERAL_PATTERNS: tuple[tuple[str, _Searchable], ...] = (
    ...
    # The OpenSSH private-key container in its DECODED form. `-----BEGIN OPENSSH PRIVATE KEY-----`
    # wraps base64 of a structure that begins with this magic, and the armor is what every pattern
    # above matches -- so the same key with its armor stripped, which is what `base64 -d` on the
    # body produces, matched nothing at all. Re-adding the header reconstructs a usable key, so the
    # decoded blob is the whole secret and was published intact.
    #
    # The magic is followed by the ciphername, kdfname and kdfoptions fields, each an SSH string
    # (a 4-byte big-endian length then its bytes). Requiring the first length to be small is what
    # keeps this off arbitrary data carrying the same 15 bytes: a real header names `none` or
    # `aes256-ctr`, so the field is a handful of bytes rather than the megabyte an accidental match
    # would declare.
    (
        "a private key",
        re.compile(rb"openssh-key-v1\x00\x00\x00\x00[\x01-\x20]"),
    ),
...
def _match(data: bytes, *, paired: bool = True) -> str | None:
    """The kind of credential the literal bytes `data` contain, or None.

    `paired=False` leaves the two-marker detectors out. They pair their halves per RECORD, and
    `search` starts a fresh splitter at depth zero -- correct for a whole file, wrong for a WINDOW
    of one, which begins mid-record. The padding record's closing brace then clamps the depth at
    zero, no boundary is seen, and two unrelated JSONL rows merge: a public JWK in one and an
    ordinary high-entropy `d` in another were reported as a private key that neither row held,
    while the same file scanned in one buffer was clean. The chunked caller pairs them itself with
    state carried across windows, so skipping them here drops a duplicate, not a check.
    """
    ...
    for kind, pattern in _LITERAL_PATTERNS:
        if not paired and isinstance(pattern, _TwoMarkers):
            continue
