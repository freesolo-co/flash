"""Parsing and validation for environment references.

Pure reference identity: the shapes a user may write for an environment (a managed ``ns/name``
slug, ``github:owner/repo@ref:path``, a github.com URL) and the safety rules every one of them
must satisfy. No network, no cache, no filesystem -- ``loader`` owns those and imports this.

Split out of ``loader`` because it is the one layer with no I/O to hold still: the parsing rules
are decided by the ref string alone, so they are testable and reviewable without a github double.
"""
