...
def spec_from_dict(raw: dict[str, Any], run_id: str | None = None) -> JobSpec:
    ...
    spec = JobSpec(
        ...
        # Most [gpu] knobs are platform-managed (the submit-time allocator picks the cheapest fitting
        # validated GPU across providers; disk is raised to the model's minimum server-side; retry
        # budget and network volumes are operator defaults). But [gpu].count is the user-selected
        # multi-GPU topology for disaggregated async GRPO (trainer + [train].inference_gpus), so it
        # IS parsed here. gpu_type is the offline sizing/display provisional, re-resolved live at
        # submit; the count>inference_gpus cross-check is in validate_topology / _validate_spec.
        gpu=GpuSpec(
            type=gpu_type,
