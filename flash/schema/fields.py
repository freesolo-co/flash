...
def _require_environment_ref(value: str, message: str) -> None:
    ...
    Delegates to the adapter's ``is_freesolo_environment_id`` acceptor — the exact grammar the
    worker uses to fetch the environment — so submit-time validation can never accept a ref the
    worker later rejects (or vice-versa).
    """
