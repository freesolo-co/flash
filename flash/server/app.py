...
async def _sweep_orphan_instances_loop() -> None:
    """Background loop: proactively tear down orphaned Lambda instances (billed VMs left
...
def _instance_providers_configured() -> bool:
    """True when an instance-based provider (Lambda) is configured on this plane, so the periodic
    instance orphan sweep is worth running. RunPod-only planes skip it — RunPod has no standing
    per-run billing to reap between restarts (its idle reaper covers warm endpoints)."""
...
def create_app():
    ...
    async def lifespan(app):
        ...
        # Periodic instance orphan sweep: proactively tear down Lambda VMs left billing by
