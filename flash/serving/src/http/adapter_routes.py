...
async def remove_adapter(
    ...
    base_record, checkpoint = await resolve_undeploy_target(context.router, adapter_id)
    ...
    result = await disable_matched([checkpoint], get_authoritative=get_authoritative)
