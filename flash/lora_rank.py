...
def resolve_adapter_ref(adapter_ref: str) -> tuple[str, str] | None:
    """Resolve ``<owner>/<repo>:<phase>/<run_id>`` into ``(repo, artifact_prefix)``."""
    match = _ADAPTER_REF_RE.fullmatch((adapter_ref or "").strip())
    if not match:
        return None
    repo = match.group("repo")
    prefix = f"{match.group('phase')}/{match.group('run_id')}"
    return repo, prefix
...
def preflight_init_adapter_lora_rank(
    ...
    *,
    ...
    """
    adapter_ref = (spec.train.init_from_adapter or "").strip()
    if not adapter_ref:
        return
    ...
    config = loader(adapter_ref, token)

    adapter_rank = rank_from_adapter_config(config, source=source)
    if adapter_rank > max_lora_rank:
        raise ValueError(
            f"train.init_from_adapter={adapter_ref!r} has rank {adapter_rank}, exceeding "
