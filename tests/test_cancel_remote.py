...
def test_cancel_deployed_run_marks_deployment_inactive(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", lambda *a, **k: [{"success": True}])
...
def test_cancel_undeploys_deployment_that_raced_in_after_entry_snapshot(tmp_path, monkeypatch):
    ...
    real_gc = runner_recovery._gc_run_resources
    ...
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", gc_then_deploy)
...
def test_cancel_loses_to_racing_genuine_completion_done(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", racing_completion)
...
def test_cancel_run_noop_when_terminal(tmp_path, monkeypatch):
    ...
    called = {"v": False}
    monkeypatch.setattr(
        runner_recovery, "_gc_run_resources", lambda *a, **k: called.__setitem__("v", True)
    )
    out = runner_deploy.cancel_run(spec.run_id)
    assert out.state == "done"
...
def test_cancel_run_accepts_confirmed_pod_delete_after_cancel_ack_failure(tmp_path, monkeypatch):
    ...
    gc_calls = []
    monkeypatch.setattr(
        runner_recovery, "_gc_run_resources", lambda value: gc_calls.append(value.run_id)
    )
...
def test_cancel_tears_down_training_before_checkpoint_serving_decision(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runner_recovery, "_gc_run_resources", lambda _spec: events.append("provider-gc")
    )
    ...
    assert events[:2] == ["provider-destroy", "provider-gc"]
    assert events[2:]
    assert set(events[2:]) == {"serving-decision"}
    assert out.state == "cancelled"
...
def test_cancel_active_removed_model_still_cleans_up_and_revokes(
    ...
    gc_calls = []
    monkeypatch.setattr(locks, "_deploy_lock", lambda _target: ContendedLock())
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", lambda _spec: gc_calls.append(_spec))
