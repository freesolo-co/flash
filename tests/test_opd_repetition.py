...
def test_c09_objective_applies_weighted_gkd_and_loop_term():
    torch = pytest.importorskip("torch")
    from flash.engine.worker import opd as opd_mod
    ...
    score = opd_mod._ScoreResult(
        teacher_toks=[TeacherToken("xxxxxx", -1.0, 0, 6)],
        status="ok",
    )
    ...
    assert model.weight.grad is not None
    assert dict(result.objective_metrics)[
        "opd/objectives/c09/repetition_severity"
    ] == pytest.approx(1.0)
