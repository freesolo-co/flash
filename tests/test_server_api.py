...
def test_recover_runs_fails_descriptorless_no_handle_run(monkeypatch, tmp_path):
    ...
    gced = []
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", lambda s: gced.append(s.run_id))
    resubmitted = []
...
def test_recover_runs_defers_resubmit_when_instance_not_confirmed_reaped(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", lambda s: None)
    resubmitted = []
    monkeypatch.setattr(runner_lifecycle, "_run_job", lambda s: resubmitted.append(s.run_id))
...
def test_recover_runs_reuses_verified_effective_snapshot_for_no_handle_resubmit(
    ...
    import flash.adapters.lora_rank as rank_mod
    import flash.server.platform.db as db_mod
    import flash.server.platform.runtime as runtime_mod
    ...
    monkeypatch.setattr(runner_lifecycle, "_run_job", fake_run_job)
    monkeypatch.setattr(runtime_mod, "_confirm_run_clear", lambda _spec: True)
...
def test_recover_runs_rejects_warmstart_artifact_drift(monkeypatch, tmp_path):
    ...
    cleaned = []
    monkeypatch.setattr(runner_recovery, "_gc_run_resources", lambda spec: cleaned.append(spec))
    monkeypatch.setattr(
        runner_lifecycle,
        "_run_job",
...
def test_recover_runs_bad_spec_is_isolated_not_fatal(monkeypatch, tmp_path):
    ...
    # The orphan sweep must still run after the loop. Malformed records still enter the known-label
    # set so the Pod sweep can clean run-labeled resources without reconstructing a JobSpec.
