...
@pytest.mark.parametrize(
    ...
    [
        (
            '{"json_object":true,"whitespace_pattern":"\\\\s*"}',
            "whitespace_pattern is unsupported",
        ),
        (
            '{"json_object":true,"disable_additional_properties":false}',
            "guidance-backend only",
        ),
        (
            '{"json":{"type":"string","format":"date-time"}}',
            "requires vLLM's guidance fallback",
        ),
        (
            '{"json":{"type":"object","properties":{"x":{"type":"number","multipleOf":2}}}}',
            "requires vLLM's guidance fallback",
        ),
    ],
)
def test_runner_rejects_structured_exactness_gaps_before_preparation(
    structured_outputs, message
):
    ...
    spec = JobSpec(
        model="Qwen/Qwen3.5-4B",
        algorithm="opd",
        train=TrainSpec(structured_outputs=structured_outputs),
    )
