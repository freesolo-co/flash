...
def _patch_opd_run_vllm_stub(monkeypatch, opd_mod, *, train_one=None, outputs=None):
    """Patch run_opd's mandatory vLLM engine with a CPU fake.

    When train_one is supplied, the fake preserves legacy harness behavior by returning that
    SampleResult from _resolve_sample after the vLLM-shaped generation/scoring phases have run.
    """
...
def test_all_skip_step_emits_stall_refresh_opd_step_heartbeat(monkeypatch):
    ...
    _patch_opd_run_vllm_stub(monkeypatch, opd_mod, train_one=_skip)  # EVERY sample skips
...
def _opd_harness(monkeypatch, *, train_one, beats=None, liveness=None, epochs=1, group=1):
    ...
    monkeypatch.setattr(opd_mod, "grad_checkpointing_on", lambda *a, **k: False)
    monkeypatch.setattr(opd_mod, "gpu_diagnostics", lambda *a, **k: {})
    _patch_opd_run_vllm_stub(monkeypatch, opd_mod, train_one=train_one)
...
def test_gkd_losses_from_logits_batch_matches_per_sample_math(monkeypatch):
    torch = pytest.importorskip("torch")
    from flash.engine.worker import opd as opd_mod
    ...
    groups0 = [([0, 1], -0.75)]
    groups1 = [([0], -0.25), ([1], -0.5)]
    chunk = [
        opd_mod._PreparedLoss(
            idx=0,
            prompt_ids=[9, 8],
            student_ids=[2, 1],
            groups=opd_mod._prepare_gkd_groups(groups0),
            coverage=1.0,
            gen_tokens=2,
            teacher_tokens=4,
            group_granularity=1.0,
        ),
        opd_mod._PreparedLoss(
            idx=1,
            prompt_ids=[7],
            student_ids=[0, 3],
            groups=opd_mod._prepare_gkd_groups(groups1),
            coverage=1.0,
            gen_tokens=2,
            teacher_tokens=3,
            group_granularity=1.0,
    ...
    losses = opd_mod._gkd_losses_from_logits_batch(
        logits, chunk, completion_suffix=False, kl_coef=0.5
    )

    assert losses is not None
    expected = [
        opd_mod._gkd_loss_from_logits_rows(logits[0, 1:3], [2, 1], groups0, kl_coef=0.5),
        opd_mod._gkd_loss_from_logits_rows(logits[1, 0:2], [0, 3], groups1, kl_coef=0.5),
    ]
    for actual, exp in zip(losses, expected, strict=True):
    ...
    monkeypatch.setenv("FLASH_OPD_BATCHED_LOGPROB_TOKEN_LIMIT", "1")
    assert (
        opd_mod._gkd_losses_from_logits_batch(
            logits.detach(), chunk, completion_suffix=False, kl_coef=0.5
        )
        is None
    )
