...
def test_c14_cost_and_vram_planning_force_k2_and_include_score_forward():
    gpu = "RTX 5090"
    ordinary = RunConfig(MID, "opd", 10, batch_size=8, group_size=1, seq_len=1024)
    c14 = RunConfig(
        MID,
        "opd",
        10,
        ...
        group_size=1,
        seq_len=1024,
        opd_objective_ids=("c14",),
    )

    assert c14.normalized().group_size == 2
    assert c14.train_knobs()["group_size"] == 2
    assert seconds_per_step(c14, gpu) > seconds_per_step(ordinary, gpu)
