        ...
        calls.append(
            {
                ...
                "local_files_only": kw.get("local_files_only", False),
            }
        )
        if (
            cache_dir
...
def test_scoped_teardown_is_runpod_only(monkeypatch):
    ...
    seen = {}
    monkeypatch.setattr(preload, "_account_storage_datacenters", lambda: ["US-CA-2"])
    monkeypatch.setattr(
        preload,
...
def test_warm_weight_cache_fans_out_over_datacenters(monkeypatch):
    ...
    def fake_one(account_index, selected_fingerprint, dc_id, *args):
        assert account_index == 0
        assert selected_fingerprint == fingerprint
        seen_dcs.append(dc_id)
        return {"account": "acct0", "datacenter": dc_id, "status": "ok"}
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            (0, fingerprint, "US-CA-2"),
            (0, fingerprint, "EU-RO-1"),
            (0, fingerprint, "US-WA-1"),
        ],
    )
...
def test_warm_weight_cache_defaults_to_full_fleet_and_catalog(monkeypatch):
    ...
    def fake_one(account_index, selected_fingerprint, dc_id, models, *args):
        assert (account_index, selected_fingerprint, dc_id) == (0, fingerprint, "US-CA-2")
        captured["models"] = models
        return {"account": "acct0", "datacenter": dc_id, "status": "ok"}

    monkeypatch.setattr(preload, "_preload_one_dc", fake_one)
    monkeypatch.setattr(preload, "_account_storage_targets", lambda: [(0, fingerprint, "US-CA-2")])
    monkeypatch.setattr(preload, "_ensure_status_repo", lambda token: None)
    results = preload.warm_weight_cache()  # no args -> all discovered DCs, whole catalog
...
def test_instance_preload_downloads_into_cache(tmp_path, monkeypatch):
    ...
    def _snap(**k):
        ...
        calls.append(k)
        snapshot = tmp_path / "hf-cache" / "hub" / "models--a--b" / "snapshots" / "abc"
...
def test_partial_datacenter_names_the_models_that_failed(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        preload,
        "_preload_one_dc",
        lambda account, selected, dc, *a, **k: {
