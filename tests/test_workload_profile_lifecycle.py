...
def test_legacy_pre_reward_latency_snapshot_still_recovers(tmp_path, monkeypatch) -> None:
    """A run prepared before reward_seconds_per_completion existed stays recoverable.

    The key is serialized on both payloads from 1.1.40 on, so a snapshot prepared before it hashed
    neither. Rehashing with it would fail a still-valid in-flight run's integrity check, blocking
    attach and reallocation across the deployment.
    """
    runner = fresh_runner(tmp_path, monkeypatch)
    spec = _spec()
    digest = _input_digest(spec)
    ...
    # the snapshot as it was written before the field existed: absent from both stored payloads.
    stored_worker = worker.to_internal_dict()
    stored_worker["train"].pop("reward_seconds_per_completion", None)
    public_spec = public.to_dict()
    public_spec["train"].pop("reward_seconds_per_completion", None)

    # The historical hash, reproduced the way the pre-1.1.40 build actually produced it: the field
    # did not exist, so it was absent from the payloads themselves. Deliberately NOT computed via
    # legacy_reward_latency=True -- that would make this digest track whatever the flag does and the
    # test would pass even with the fix removed.
    class _PreRewardLatency:
        """A spec whose serialization predates ``reward_seconds_per_completion``."""
        ...
        def to_dict(self) -> dict:
            data = self._inner.to_dict()
            data["train"] = {
                k: v for k, v in data["train"].items() if k != "reward_seconds_per_completion"
            }
            return data

        def to_internal_dict(self) -> dict:
            data = self._inner.to_internal_dict()
            data["train"] = {
                k: v for k, v in data["train"].items() if k != "reward_seconds_per_completion"
            }
            return data

    historical_digest = runner._preparation_digest(
        _PreRewardLatency(JobSpec.from_dict(public_spec)),
        _PreRewardLatency(JobSpec.from_dict(stored_worker)),
        None,
    )
    status = runner.RunStatus(
        run_id=worker.run_id,
        ...
        effective_preparation={
            ...
            "workload_profile": profile,
            "adapter_identity": None,
            "preparation_digest": historical_digest,
        },
    )
    # guard: the stored bytes really are the pre-upgrade shape
    assert (
        "reward_seconds_per_completion" not in status.effective_preparation["worker_spec"]["train"]
    )
    assert runner.effective_spec_from_status(status) == worker
