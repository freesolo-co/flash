...
def _dispatch_record(*, deployment_generation: str | None = "generation-1") -> AdapterRecord:
    """A real ready record, since the pool forwards ``internal_adapter_payload(record)``."""
...
def test_warm_pool_dispatches_inference_and_registration(modal_app_module, monkeypatch):
    ...
    expected_record_dict = internal_adapter_payload(record)
    ...
    assert register_calls == [(expected_record_dict, "generation-1")]
...
def test_modal_stream_returns_channel_directly_and_close_reaches_channel(
    ...
    assert captured == {
        ...
        "record_dict": internal_adapter_payload(record),
