...
def test_9b_has_b200_rank128_serving_overrides() -> None:
    ...
    # 16 decode slots, not 8: the per-seq cost on this hybrid model is its constant-size GatedDeltaNet
    # recurrent state, so the extra slots are sub-GiB on a 180 GiB card. modal admission is sized 1:1
    # off this number, so it is the container's throughput.
...
def test_qwen38_27b_is_active_on_b200_with_pinned_immutable_revisions() -> None:
    ...
    # upstream retag. The rank-64 x 16 LoRA shape at 32k was sized on the H100 (80 GiB); the 180 GiB
    # B200 it now serves on is a strict superset of that fit.
    ...
    assert ov["max_model_len"] == 32768
    # 16 decode slots, matching the 9B: dense, so no MoE activation spike at profiling time.
    assert ov["max_num_seqs"] == 16
    assert ov["gpu_memory_utilization"] == 0.90
