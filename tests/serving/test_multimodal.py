...
def test_text_only_tool_history_skips_image_decoding(monkeypatch) -> None:
    ...
    assert pool.calls == [(QWEN_27B, REVISION_ID, REVISION_ID)]
...
def test_all_message_entry_points_validate_against_resolved_model(
    ...
    assert supports_args == [QWEN_27B]
    assert limit_args == [QWEN_27B]
    # and multimodal validation was invoked exactly once with those resolved capabilities.
    assert validated == [
        {"supports_images": real_supports(QWEN_27B), "image_limit": real_limit(QWEN_27B)}
    ]
    assert len(pool.calls) == 1
    assert pool.calls[0][0] == QWEN_27B
