...
def test_push_single_py_ships_sibling_datasets_plural(monkeypatch, tmp_path):
    # ``datasets`` (plural) is the convention users reach for by hand; it must ship too,
    # otherwise a mismatch with the ``dataset`` scaffold silently drops the training rows.
    env_file = tmp_path / "environment.py"
    env_file.write_text("def load_environment(**k):\n    return None\n")
