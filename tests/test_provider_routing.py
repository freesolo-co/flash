...
def test_success_preserves_uncanonical_cleanup_sibling_and_returns_metrics(orch, monkeypatch):
    ...
    def fake_runpod_submit(run_spec, log=None, on_handle=None, attempt=0, **_kwargs):
...
def test_success_attempts_teardown_when_cleanup_recording_fails(
    ...
    monkeypatch.setattr(attempt_supervision._lifecycle, "_strict_teardown_handle", fake_teardown)
...
def test_new_provider_handle_replaces_retained_accounting_identity(orch):
    from flash.runner.supervise.attempt_supervision import _build_context
    ...
    ctx = _build_context(spec, io.StringIO(), None, _SOURCE_SNAPSHOT, 1)
