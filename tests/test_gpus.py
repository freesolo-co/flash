...
def test_flash_gpu_enum_members():
    ...
    # B200/H200 are newly added; resolving them fails fast (AttributeError) if the installed
    # runpod_flash SDK doesn't expose the matching enum member, so CI catches an SDK/version gap
    # before a live allocation tries to provision the card.
    assert flash_gpu("B200").name == "NVIDIA_B200"
    assert flash_gpu("H200").name == "NVIDIA_H200"
