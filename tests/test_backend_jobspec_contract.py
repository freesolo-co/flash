...
map to a valid autoslm JobSpec and round-trip.
...
That same config shape is what autoslm parses into a JobSpec
(autoslm/autoslm/schema.py spec_from_dict -> autoslm/autoslm/spec.py JobSpec). If the
backend accepts a config autoslm rejects (or the two drift on field names), a run the
backend admits never trains. This test pins that seam.

Both packages import in the autoslm venv (the backend schemas are pure pydantic), so
...
def _representative_config() -> dict:
    ...
    SDK/agent author into autoslm.toml."""
    return {
...
def test_backend_run_config_parses_into_valid_jobspec() -> None:
    ...
    spec = spec_from_dict(_representative_config(), run_id="autoslm-test-1")
    ...
    assert spec.run_id == "autoslm-test-1"
...
def test_jobspec_round_trips_through_dict() -> None:
    ...
    spec = spec_from_dict(_representative_config(), run_id="autoslm-test-2")
...
def test_backend_config_through_runcreaterequest_then_jobspec() -> None:
    ...
    the validated config into autoslm's parser. A config the backend accepts must
    produce a JobSpec.
    """
    try:
    ...
    spec = spec_from_dict(req.config, run_id="autoslm-test-3")
    assert spec.environment.id == "owner/my-env"
    assert spec.algorithm == "grpo"
...
def test_sft_backend_config_maps_to_jobspec() -> None:
    ...
    spec = spec_from_dict(config, run_id="autoslm-sft-1")
...
def test_backend_config_missing_required_fields_is_rejected_consistently() -> None:
    ...
    (no [environment] id, or no train.hf_repo) must be rejected by autoslm with a
    ConfigError (-> HTTP 400 on the control plane), not silently accepted. This keeps
    the backend/agent and autoslm agreeing on what a *complete* job spec is.
    """
    no_env = {
        "model": "Qwen/Qwen3.5-4B",
        "algorithm": "grpo",
