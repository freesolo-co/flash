...
Read paths only: API auth/reachability (the instances list), the verified-datacenter offer search,
and the offer->class mapping / live pricing. Never rents an instance. Gated on VAST_API_KEY +
FLASH_LIVE (the live marker), like the Lambda/RunPod live smokes.
"""
...
def test_vast_usable_offers_and_pricing():
    ...
    # the provider interface returns a positive rate (cheapest live offer, else static fallback)
    assert get_provider("vast").hourly_rate("RTX 4090") > 0
