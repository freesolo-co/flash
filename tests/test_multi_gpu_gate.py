...
def test_gate_helper_allows_verl_algorithms_and_rejects_unmigrated_algorithms():
    from flash import runner

    assert runner._require_supported_gpu_count(_spec(2, "sft")) is None
    assert runner._require_supported_gpu_count(_spec(2, "opd")) is None
    with pytest.raises(ValueError, match="multi-gpu training"):
        runner._require_supported_gpu_count(_spec(2, "grpo"))
...
def test_submit_job_rejects_multi_gpu_at_boundary(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(ValueError, match="multi-gpu training"):
            runner.submit_job(_spec(2, "grpo"), dry_run=True)
...
def test_run_training_gates_effective_spec_on_recovery():
    ...
    with pytest.raises(ValueError, match="multi-gpu training"):
        _run_training(_spec(2, "grpo"), io.StringIO(), prior_cost=0.0)
