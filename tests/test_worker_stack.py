...
def test_capsule_builder_copy_inventory_builds_a_nonempty_capsule(tmp_path):
    import os
    import shlex
    import shutil
    import subprocess
    ...
    dockerfile = (root / "Dockerfile.worker").read_text()
    builder = dockerfile.split("FROM python:3.12-slim AS capsule-builder", 1)[1].split(
        "\nFROM ", 1
    )[0]
    copy_lines = [line for line in builder.splitlines() if line.startswith("COPY ")]
    assert any("flash/_internal/version.py" in line for line in copy_lines)
    ...
    for line in copy_lines:
        ...
        target = (
            tmp_path / destination.removeprefix("/")
            if destination.startswith("/")
            else stage / destination
        )
        for source_name in sources:
            source = root / source_name
            if source.is_dir():
                shutil.copytree(source, target, dirs_exist_ok=True)
            elif len(sources) > 1 or destination.endswith("/"):
    ...
    destination = tmp_path / "instance-bootstrap.pyz"
    result = subprocess.run(
        [
            sys.executable,
            str(stage / "docker" / "build_instance_capsule.py"),
            str(stage),
            str(destination),
        ],
        check=False,
