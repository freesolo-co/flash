...
def test_periodic_console_snapshot_cannot_clobber_the_terminal_one(tmp_path, monkeypatch):
    ...
    assert live_path == str(console) + ".live.tail"
    assert terminal_path == str(console) + ".terminal.tail"
    assert (live_name, terminal_name) == ("console_sft_live.txt", "console_sft.txt")
    # only the terminal artifact carries the wall-clock-cap evidence, and it survives intact.
    terminal_tail = (tmp_path / "console_sft.txt.terminal.tail").read_text()
    live_tail = (tmp_path / "console_sft.txt.live.tail").read_text()
...
def test_console_snapshot_redacts_a_payload_env_secret(tmp_path, monkeypatch):
    ...
    tail = (tmp_path / "console_sft.txt.live.tail").read_text()
    ...
    assert uploads == [(str(console) + ".live.tail", "console_sft_live.txt")]
