...
Every protected request carries `X-Freesolo-Internal-Key`. Compare it with `hmac.compare_digest` and
reject a mismatch with `401` or `403`. The client removes this header on cross-origin redirects and
keeps it on same-origin redirects.
...
```json
{ "ok": true, "base_models": ["Qwen/Qwen3.5-4B"], "requires_key": true, "capabilities": ["immutable_adapter_revisions", "alias_compare_and_swap"] }
```
...
```json
{ "adapter_id": "run-abc@step-10.<40-hex>", "repo_id": "acme/artifacts", "repo_type": "dataset", "subfolder": "sft/run-abc/adapter", "base_model": "Qwen/Qwen3.5-4B", "checkpoint": "run-abc/step-10", "org_id": "org-id", "thinking": false, "structured_outputs": { "regex": "[ab]+" }, "metadata": { "record_type": "revision", "run_id": "run-abc", "checkpoint_step": 10, "hf_revision": "<40-hex>" } }
```
...
```json
{ "adapter_id": "run-abc", "target_adapter_revision": "run-abc@step-10.<40-hex>", "previous_adapter_revision": null, "checkpoint": "run-abc/step-10", "updated_at": "2026-08-12T00:00:00Z" }
```
...
```json
{ "run_id": "run-abc", "disabled_aliases": ["run-abc"], "disabled_revisions": ["run-abc@step-10.<40-hex>"] }
```
...
## Conformance
Run against a backend you own because the suite registers, activates, chats, and deletes real state:
```bash
